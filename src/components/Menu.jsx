...
export default function Menu(){
    ...
    //Función para borrar pizzas y actualizar estado
    const deletePizza = (pizzaName) => setPizzas((prev) => prev.filter((pizza) => pizza.name !== pizzaName));
        return(
            <main className="menu">
                <h2>Our Menu</h2>
                {pizzas.length> 0 ?
                //<> </> Se conoce como fragment, y no se renderiza en la aplicación, nos sirve como contenedor padre pero sin ser redendirada en el html
                (<>
                    <p>
                        Authentic Italian cousine. 6 creatives dishes to choose from. All from our stone oven, all organic all delicicius.
                    </p>
                    <ul className="pizzas">
                        {pizzas.map((pizza)=> {
                            return(
